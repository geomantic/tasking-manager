...
class ProjectService:
    ...
    def get_project_dto_for_mapper(
        ...
        is_manager = ProjectAdminService.is_user_action_permitted_on_project(
            current_user_id, project_id
        )
        # Draft Projects - admins, authors, org & team managers permitted
        if project.status == ProjectStatus.DRAFT.value:
            if not is_manager:
                is_allowed_user = False

        # Private Projects - allowed_users, admins, org & team managers, authors permitted
        if project.private and not is_manager:
            is_allowed_user = (
                len(
                    [
                        user
        ...
        if is_allowed_user or is_manager:
